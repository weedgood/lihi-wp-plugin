...
- `lihi-shorturl/includes/config.php` — returns a flat array of plugin configuration values (`api_domain`, `redirect_domain`, `auth_domain`). Loaded lazily by `lihi_config()` and cached for the request lifetime.
...
- `lihi-shorturl/includes/settings.php` — registers the plugin settings page under Settings → lihi Short URL. The "Save & Verify" button fires `wp_ajax_lihi_update_email` (named function `Lihi\ShortUrl\ajax_update_email()`) which first calls `Lihi_Auth_Client::update_email( $email )` and only persists the option (`update_option('lihi_email', ...)`) when the auth service accepts the email, so an invalid email never becomes the active configuration. The response payload is `{ verified: bool }` so the UI can show "verified" vs. "verification email sent"; `Lihi_Validation_Exception` / `Lihi_Server_Exception` map to localized error messages (raw message logged via `error_log` on server errors). Page is always available regardless of whether email is configured.
...
- `lihi-shorturl/includes/client/lihi-auth-client-interface.php` / `lihi-auth-client.php` — `Lihi_Auth_Client_Interface` and `Lihi_Auth_Client` for the lihi auth service (base URL `lihi_config( 'auth_domain' )`). Exposes `update_email( $email )` (POST `/auth/update-email`) and `login( $email )` (POST `/auth/login`). Every request overrides the HTTP `Host` header with the WP site's host (from `home_url()`) because the auth service identifies the tenant from that header. Errors map to existing exceptions: 400 → `Lihi_Validation_Exception`, 403 → `Lihi_Auth_Exception`, 500 / network → `Lihi_Server_Exception`.
...
- `lihi-shorturl/assets/lihi-settings.js` — handler for the settings page "Save & Verify" button. POSTs email to `lihi_update_email` AJAX endpoint, then renders an inline `notice` below the input: `.notice-success` with the verified / sent label, or `.notice-error` with the server error message (text-only, `textContent`, never `innerHTML`).
