...
- **Settings page** — configure the lihi API email under Settings → lihi Short URL; once an email is verified, the page also shows the account's plan tier and plan end date. The lihi button appears whenever an email is configured; redirect-domain and UTM source / medium selection happens in the create modal from the account options API.
...
├── assets/
│   ├── lihi-button-api.js     Frontend Short URL API facade; encapsulates admin-ajax action/nonce payloads for options/create/copy/edit, JSON error parsing, and a 60-second domain / UTM options cache
│   ├── lihi-button-modal.js   Centered fade create / notice / confirm modal rendering and interaction; loads domain and UTM source / medium options, renders default and removable user tag chips in one Tags field, and collects UTM fields
...
└── includes/
    ...
    ├── client/
    │   ├── lihi-client-interface.php       Unified lihi Wordpress API contract; covers auth (update_email, login) plus JWT endpoints (get_profile, get_options, create_passthrough_nonce, get_sites, get_short_links, create_site)
    ...
    └── service/
        └── lihi-service.php            Business logic: login($email), get_profile(), get_url_options(), get_or_create_short_url(), get_existing_short_url(), create_passthrough_nonce(); client and token store are injected by helper.php; config is read by helper.php, not by the service
```
...
4. The service checks for an existing short link via `get_short_links()` (`GET /site/find`, reading `data.site`); creates one with `create_site()` (`POST /site/store`) if none is found. The create payload merges default tags with user tags, sends them to lihi as a comma-separated string, and appends UTM parameters directly to the destination URL.
