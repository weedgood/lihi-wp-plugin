...
// Show a setup notice when the plugin has not been fully configured yet.
// Feature hooks in add-shorturl-column.php guard themselves on both
// lihi_email() AND lihi_domain(); the AJAX handler is always registered so
// stale buttons get a friendly error instead of WordPress's bare "0" response.
if ( lihi_email() === '' ) {
    ...
    add_action( 'admin_notices', function () {
        ...
        echo '<div class="notice notice-warning"><p>'
            . wp_kses(
                sprintf(
                    /* translators: %s: URL to the lihi Short URL settings page */
                    __( 'lihi Short URL: please <a href="%s">choose a redirect domain</a> to enable the plugin.', 'lihi-shorturl' ),
                    $url
                ),
                [ 'a' => [ 'href' => [] ] ]
            )
